...
{
    //std::cout << "RadioBearerStatsCalculator::WriteUlResults(std::ofstream& outFile)" << std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout << "RadioBearerStatsCalculator::WriteDlResults(std::ofstream& outFile)" << std::endl;
    NS_LOG_FUNCTION(this);
    ...
    for(auto it = m_flowId.begin(); it != m_flowId.end(); it++) {
        if (find(pairVector.begin(), pairVector.end(), (*it).first) == pairVector.end())
        {
            pairVector.push_back((*it).first);
        }
    ...
    for (std::vector<ImsiLcidPair_t>::iterator pair = pairVector.begin(); pair != pairVector.end();
         ++pair)
    {
        //std::cout << "chelik: " << (*pair).m_imsi << std::endl;
        // std::cout << (*pair).m_lcId << std::endl;
        ...
        if(m_dlRxPackets.find(p) == m_dlRxPackets.end()) {
            outFile << m_startTime.GetSeconds() << "\t";
            outFile << endTime.GetSeconds() << "\t";
