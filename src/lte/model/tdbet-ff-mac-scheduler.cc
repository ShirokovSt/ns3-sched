    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    //МЕТРИКА тут идем по всем пользователям и выделяем блоки
    for (it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++)
    {
        // check first what are channel conditions for this UE, if CQI!=0
        std::map<uint16_t, uint8_t>::iterator itCqi;
        //CQI как и у всех, тут 1-0, чем она лучше - тем лучше условия канала у абонента
        ...
        //если у пользователя есть уже блоки, то ничего не выделяем
        ...
        //как раз метрика из лекции, берем среднюю скорость абонента, 1 делим на неё и выбираем по максимуму кому отдаль все блоки
    ...
    {
        //отдаём все RBG абоненту у которого была самая большая метрика
