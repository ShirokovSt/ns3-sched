    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    //МЕТРИКА
    //аналогично идём по абонентам, нет цикла по блокам, так как все блоки будут 100% отданы одному абоненту
    for (it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++)
    {
        //если уже выделено что-то абоненту, то уходим
        ...
        //а тут периодичный отчёт получаем о CQI по UL, 1-0 (от одного пользователя???) - то есть информацию о канале
        // -модуляция используемая, скорость кода, эффективность, чем он больше - тем лучше условия канала у пользователя!
        std::map<uint16_t, uint8_t>::iterator itCqi = m_p10CqiRxed.find((*it));
        uint8_t wbCqi = 0;
        //аналогично в wbCqi записываем этот параметр доя текущего пользователя
        ...
        //аналогично если он 0, то что-то не так
        if (wbCqi != 0)
        {
            // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
            //смотрим, есть ли что пользователю передавать вообще
            if (LcActivePerFlow(*it) > 0)
            {
                // this UE has data to transmit
                //Это как раз нужная нам метрика для MT, которая есть в лекции, для каждого абонента
                ...
                //а тут получается эту метрику как раз ставим на максимум
                if (metric > metricMax)
                {
                    metricMax = metric;
                    itMax = it;
    ...
    else
    {
        ...
        //теперь если есть что передавать, то отдаём все 12 групп RB (в общем 24 ресурсных блока) - одному абоненту!
