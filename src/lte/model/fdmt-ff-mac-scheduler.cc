    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    //МЕТРИКА - а вот в частотном RB мы идём по группам блоков
    for (int i = 0; i < rbgNum; i++)
    {
        NS_LOG_INFO(this << " ALLOCATION for RBG " << i << " of " << rbgNum);
        //если RBG - свободен - идём по нему
        if (rbgMap.at(i) == false)
        {
            ...
            for (it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++)
            {
                ...
                //если нечего передавать абоненту - выходим
                ...
                //получаем информация об отчётности CQI 3-0 (от 3-ёх абонентов???)
                ...
                //если ничего нет - то самое маленькое значение добавляем
                ...
                //главное чтобы оно не 0 было
                if ((cqi1 > 0) ||
                    (cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
                {
                    //если есть что передавать - выделяем
                    if (LcActivePerFlow((*it)) > 0)
                    {
                        ...
                        NS_LOG_INFO(this << " RNTI " << (*it) << " MCS " << (uint32_t)mcs
                                         ...
                                         << rcqi);
                        //если метрика больше получилось - ставим её
                        if (rcqi > rcqiMax)
                        {
                            rcqiMax = rcqi;
                            itMax = it;
