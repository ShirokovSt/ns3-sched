    ...
    const struct FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
    ...
    //вот это у нас будет как раз для QoS!!!
    ...
    else
    {
        ...
        {
            LteFlowId_t flowId = LteFlowId_t(params.m_rnti, lcit->m_logicalChannelIdentity);
            for (unsigned int i = 1; i <= m_ueLogicalChannelsConfigList.size(); i++) {
                LteFlowId_t tmpFlow = LteFlowId_t(flowId.m_rnti, i);
                m_ueLogicalChannelsConfigList.erase(tmpFlow);
                RRforUE.erase(tmpFlow.m_rnti);
            }
            m_ueLogicalChannelsConfigList.insert(
                std::pair<LteFlowId_t, LogicalChannelConfigListElement_s>(flowId, *lcit));
            RRforUE.insert(std::pair<uint16_t, int>(flowId.m_rnti, 0));
        }
    }
...
// bool comparatorTds (const std::pair<uint16_t, double>& left, const std::pair<uint16_t, double>& right)
// {
    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    // //----------------РАССКОМЕНТИТЬ
    auto itQOS = m_ueLogicalChannelsConfigList.begin();
    std::map<uint16_t, struct LogicalChannelConfigListElement_s>  rntiSAndQoSParams;
    if(VoipMOD) {
    //bool flagExistsQos1 = false;
    for (itQOS = m_ueLogicalChannelsConfigList.begin(); itQOS != m_ueLogicalChannelsConfigList.end(); itQOS++) {
        rntiSAndQoSParams[(*itQOS).first.m_rnti] = (*itQOS).second;
        // if(rntiSAndQoSParams[(*itQOS).first.m_rnti].m_qci == 1) {
        //     flagExistsQos1 = true;
        // }
    
    }
    // if(Simulator::Now() == Seconds(2.010) || Simulator::Now() == Seconds(2.020) || Simulator::Now() == Seconds(2.030)) { //&& Simulator::Now() < Seconds(7.0)) {
    //     for(auto it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
    //         if(rntiSAndQoSParams[(*it).first.m_rnti].m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR ) {
    //             (*it).second.m_rlcTransmissionQueueSize = 1;
    //             (*it).second.m_rlcTransmissionQueueHolDelay = 1;
    //             break;
    //         }
    //     }
    //     //(*m_rlcBufferReq.begin()).m_rlcTransmissionQueueSize = 1;
    // }
    // else {
    //     for(auto it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
    //         if(rntiSAndQoSParams[(*it).first.m_rnti].m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR ) {
    ...
    //         }
    //     }
    //     //(*m_rlcBufferReq.begin()).m_rlcTransmissionQueueSize = 0;
    // }
    if((((int)Simulator::Now().GetMilliSeconds() % 2) == 0) && (int)Simulator::Now().GetMilliSeconds() > 60) {
        //  for(auto it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
        //     if(rntiSAndQoSParams[(*it).first.m_rnti].m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR ) {
        //         (*it).second.m_rlcTransmissionQueueSize = 1;
        //         (*it).second.m_rlcTransmissionQueueHolDelay = 1;
    ...
    for(auto it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
            if(rntiSAndQoSParams[(*it).first.m_rnti].m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR ) {
                (*it).second.m_rlcTransmissionQueueSize = QueueSize;
                (*it).second.m_rlcTransmissionQueueHolDelay = HOL;
                break; 
            }
    }
    }
    ...
    if(Simulator::Now() == Seconds(3.150)  || Simulator::Now() == Seconds(3.158) || Simulator::Now() == Seconds(3.159) )
        std::cout << "JPJ" << std::endl;

    std::map<uint16_t, fdbetsFlowPerf_t>::iterator itFlow;
    //в данном переменной будет храниться ожидаемая скорость для UE
    std::map<uint16_t, double> estAveThr; // store expected average throughput for UE
    //----------------РАССКОМЕНТИТЬ
    std::map<uint16_t, double> tdsMetrics;// тут будет хранится метрика дла TDS
    //----------------РАССКОМЕНТИТЬ
    std::map<uint16_t, double>::iterator itMax = estAveThr.end();
    std::map<uint16_t, double>::iterator it;
    //а тут будет номер RGB, который присвоен UE
    ...
    //std::vector<uint16_t> forVoip = {1, 2, 3, 4, 5, 6, 7, 9};
    //auto isForVoip = std::count(forVoip.begin(), forVoip.end(), (0xF & params.m_sfnSf) );
    //----------------
    
    //МЕТРИКА
    //идем по абонентам
    //----------------РАССКОМЕНТИТЬ
    int Nmax = 4; //максимальное кол-во абонентов в TTI
    double summRA_forVoipUe; // сумма VoIP RA
    //----------------РАССКОМЕНТИТЬ
    for (itFlow = m_flowStatsDl.begin(); itFlow != m_flowStatsDl.end(); itFlow++)
    {
        ...
        //если абоненту нечего передавать - уходим
        ...
        //если CQI не 0, то в estAveThr вставляем абонента и ожидаемую скорость
        if (cqiSum != 0)
        {       
            if(VoipMOD) {
            // //----------------РАССКОМЕНТИТЬ
                RRforUE[(*itFlow).first]++;
                double rrCoef = RRforUE[(*itFlow).first];
                double RAcoef = 1.0;
                double DS = 1.0;
                if (rntiSAndQoSParams[(*itFlow).first].m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR && rntiSAndQoSParams[(*itFlow).first].m_qci == 1) {
                    RAcoef = 37500 / (double)(*itFlow).second.lastAveragedThroughput;
                    for(auto itBuffer = m_rlcBufferReq.begin(); itBuffer != m_rlcBufferReq.end(); itBuffer++) {
                        if((*itBuffer).first.m_rnti == (*itFlow).first) {
                            DS = pow(2, (*itBuffer).second.m_rlcTransmissionQueueHolDelay - 1);
                            if((*itBuffer).second.m_rlcTransmissionQueueSize == 0)
                            DS = 0;
                            break;
                        }
                    }
                    summRA_forVoipUe += RAcoef;
                }

                double metricTds = rrCoef * RAcoef * DS;
                //записываем метрику
                tdsMetrics.insert(std::pair<uint16_t, double>((*itFlow).first,
                                                         metricTds));
            }
                 // //----------------РАССКОМЕНТИТЬ
    ...
    // //----------------РАССКОМЕНТИТЬ
    //смотрим сколько  не VoIP всего
    auto countQOSVoip = std::count_if(rntiSAndQoSParams.cbegin(), rntiSAndQoSParams.cend(), [](std::pair<uint16_t, struct LogicalChannelConfigListElement_s> el) {
        return el.second.m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR && el.second.m_qci == 1; });
    
    auto Nbe = m_flowStatsDl.size() - countQOSVoip;
    uint16_t VoIPUSER = -1;
    std::multimap<double, uint16_t> tdsValues;
    if(VoipMOD) {
    for (auto itUe = tdsMetrics.begin(); itUe != tdsMetrics.end(); itUe++)  {
        if (rntiSAndQoSParams[(*itUe).first].m_qosBearerType != LogicalChannelConfigListElement_s::QBT_GBR && rntiSAndQoSParams[(*itUe).first].m_qci != 1) {
            (*itUe).second = (*itUe).second  * std::max(0.0,(Nmax - summRA_forVoipUe) / Nbe);
            tdsValues.insert(std::pair<double, uint16_t>((*itUe).second, (*itUe).first));
        } else {
            tdsValues.insert(std::pair<double, uint16_t>((*itUe).second, (*itUe).first));
            VoIPUSER = (*itUe).first;
        }
    ...
    // std::qsort(tdsValues)
    // std::qsort(tdsValues.begin(), tdsValues.end());//, comparatorTds);
    if(Simulator::Now() == Seconds(2.039) )
        std::cout << "bad";
    if(VoipMOD) {
    if(Nmax < (int)tdsValues.size()) {
        auto itTds = tdsValues.rbegin();
        std::advance(itTds, Nmax);
        while (itTds != tdsValues.rend())
        {
            estAveThr.erase((*itTds).second);
            itTds++;
        }        
    }
    if(estAveThr.find(VoIPUSER) != estAveThr.end()) {
            if(QueueSize != 0)
                QueueSize--;
            if(HOL != 0)
                HOL--;
    ...
    // //----------------РАССКОМЕНТИТЬ
    //идем по абонентам, которые были добавлены в estAveThr (абонент - ожидаемая скорость)
    if (estAveThr.size() != 0)
    {
        // Find UE with largest priority metric
        //здесь ищем абонента у которого будет самая большая метрика для BET
        for (it = estAveThr.begin(); it != estAveThr.end(); it++)
        {
            // //----------------РАССКОМЕНТИТЬ
            if(VoipMOD) {
            RRforUE[(*it).first] = 0;
            //определение метрикиа для BET как в лекции
            //----------------
            auto cqiForIT = m_p10CqiRxed.find((*it).first);
            double achievableRate = 0.0;
            uint8_t mcs2 = 0;
            if (cqiForIT == m_p10CqiRxed.end())
            {
                mcs2 = 0; // no info on this user -> lowest MCS
            }
            else
            {
                mcs2 = m_amc->GetMcsFromCqi((*cqiForIT).second);
                if((*it).first == VoIPUSER)
                    mcs2 = 6;
            }
            achievableRate += ((m_amc->GetDlTbSizeFromMcs(mcs2, rbgSize) / 8) /
                            0.001); // = TB size / TTI

            double metric = achievableRate / (*it).second;
            if (metric > metricMax)
            {
                itMax = it;
                metricMax = metric;
            ...
            //как видно, вставляем всегда UE и 1 для каждого абонента
             // //----------------ЗАККОМЕНТИТЬ
        ...
        int i = 0;
        //начинаем выделения блоков для абонента, делая всё возможное для уравнивания скоростей
        //цикл идет по всем RBG, метрика постоянно пересчитывается
        do
        {
            NS_LOG_INFO(this << " ALLOCATION for RBG " << i << " of " << rbgNum);

            //если RBG свободен, выделяем его
            if (rbgMap.at(i) == false)
            {
                ...
                //находим для текущего абонента в табличке запись: UE - кол-во выдлененых RBG
                itRbgPerRntiLog = rbgPerRntiLog.find((*itMax).first);
                std::map<uint16_t, fdbetsFlowPerf_t>::iterator itPastAveThr;
                //находим этого абонента, каким он был в прошлый TTI
                ...
                //--------------------
                //int tbSizeFoQos = bytesTxed;
                //--------------------
                //рассчитываем среднюю скорость для текущего абонента, похожа на метрику, которая
                //описывалась в лекции, которая с бета и альфа, надо проверить
                ...
                //добавляем пользователю в его табличку ещё один RGB
                int rbgPerRnti = (*itRbgPerRntiLog).second;
                rbgPerRnti++;
                rbgPerRntiLog[(*itMax).first] = rbgPerRnti;
                //обновляем среднюю скорость для текущего абонента
                estAveThr[(*itMax).first] = expectedAveThr;

                // find new UE with largest priority metric
                //теперь ищем нового пользователя, у которого метрика наиболее приоритетна
                metricMax = 0.0;
                //идем по средним скоростям и смотрим у кого 1метрика (1 / средняя скорость) больше всего
                for (it = estAveThr.begin(); it != estAveThr.end(); it++)
                {
                    ...
                    for (uint8_t j = 0; j < nLayer; j++)
                    {
                        ...
                        if((*it).first == VoIPUSER) {
                            if(fl == 0)
                                continue;
                            int tbSizeCheck2 = 0;
                            if(rbgPerRntiLog[(*it).first] - 1 > 0) 
                                tbSizeCheck2 = (m_amc->GetDlTbSizeFromMcs(mcs2, (rbgPerRntiLog[(*it).first] - 1) * rbgSize) /
                            8);
                            int tbSizeCheck = (m_amc->GetDlTbSizeFromMcs(mcs2, rbgPerRntiLog[(*it).first] * rbgSize) /
                            8);
                            if(tbSizeCheck >= sizeOfData && tbSizeCheck2 >= sizeOfData && fl != 0) {//&& (fl == 1  || fl == 0)) {
                                fl = 0;
                                continue;
                            }
                            else if(fl != 0) 
                                fl = 1;
                            if(tbSizeCheck >= sizeOfData && tbSizeCheck2 >= sizeOfData && fl == 2) {
                                sizeOfData = INT32_MIN;
                                fl = 0;
                                continue;
                            }
                        }
                    //получаем размер транспортного блока / TTI = достижимый показатель
                     //----------------РАССКОМЕНТИТЬ
                     //----------------РАССКОМЕНТИТЬ
                    if(VoipMOD) {
                    double metric = achievableRate / (*it).second;
                    // if((fl == 1) && (fl != 0) && estAveThr.find(VoIPUSER) != estAveThr.end() && ((*itMax).first == VoIPUSER)) {
                    //     itMax = estAveThr.find(VoIPUSER);
                    // } else
                    if (metric > metricMax)
                    {
                        itMax = it;
    ...
    while (itMap != allocationMap.end())
    {
        ...
        for (uint8_t j = 0; j < nLayer; j++)
        {
            ...
            {
                if((*itMap).first == VoIPUSER)
                    newDci.m_mcs.push_back(6);
                    else
            ...
            if((*itMap).first == VoIPUSER) {
                ...
                if (fl == 1) {
                    sizeOfData = VOIP_PACKET - tbSize;
                } else
                if(sizeOfData <= tbSize) {
                    tbSize = sizeOfData;
                } else sizeOfData = sizeOfData - tbSize;
                if(fl == 0) {
                    sizeOfData = VOIP_PACKET;
                }
                if(sizeOfData == 0)
                    std::cout << "bad";
            } 
            }
            if(Simulator::Now() >= Seconds(2.000) && Simulator::Now() < Seconds(2.040)) {
                std::cout << "tbSize " << tbSize << std::endl; 
        ...
        newDci.m_rbBitmap = 0; // TBD (32 bit bitmap see c of 36.213)
        uint32_t rbgMask = 0;
