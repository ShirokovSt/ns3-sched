    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    //по сути цикл выделения блоков для RNTI - МЕТРИКА
    //идём по блокам, т.к у нас ширина полосы частот для dl 25 блоков, то rbgNum - 2 ресурсных блока, то бишь только 24 будут исп
    ...
    for (int i = 0; i < rbgNum; i++)
    {
        ...
        //если группа ресурсных блоков уже использована, ничего не делаем, иначе производим выделение
        if (rbgMap.at(i) == false)
        {
            std::map<uint16_t, pfsFlowPerf_t>::iterator it;
            std::map<uint16_t, pfsFlowPerf_t>::iterator itMax = m_flowStatsDl.end();
            double rcqiMax = 0.0;
            //m_flowStatsDl - по сути, характеристики об абонентах, для распределения РБ группы конкретному абоненту
            //информация следующая:
            // - общее кол-во переданных байт
            // - кол-во переданных байт последнего временного интервала передачи
            // - последняя усреднённая пропускная способность (по сути средняя скорость!!)
            for (it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++)
            {
                //если данная ресурсная группа для данного пользователя недоступна - выходим!
                if ((m_ffrSapProvider->IsDlRbgAvailableForUe(i, (*it).first)) == false)
                {
                    continue;
                }
                //ищем, вдруг данный RNTI уже имеет РБ группы и ему не надо ничего давать
                ...
                //получаем апериодичный  3-0 (3-ём пользователям может быть выделено???)отчёт по UL о CQI (чем больше CQI у блока - чем больше скорость у абонента, значения от 0 до 15)
                itCqi = m_a30CqiRxed.find((*it).first);
                std::map<uint16_t, uint8_t>::iterator itTxMode;
                //режим передачи для абонента, у нас всгде такой что nLayer всегда 1
                ...
                //если не получен CQI об абоненте, добавляем в sbCqi самое маленькое значение - 1
                ...
                {   //не очень понятно, что это пока, но тоже что-то с CQI
                ...
                //в общем, если эти CQI не 0 - то всё ок, как я понимаю они помогают определать схему передачи DL
                if ((cqi1 > 0) ||
                    (cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
                {
                    //смотрим, есть ли у абонента данные для передачи
                    if (LcActivePerFlow((*it).first) > 0)
                    {
                        ...
                        //теперь делим достижимый показатель на среднюю пропуснкую скорость, по сути это и есть формула из лекции
                        
                        // double koef = 0.0;

                        // double rcqi = 0.0;

                        // if(rntiSAndQoSParams[(*it).first].m_qci != 1) {
                        //     if((*it).second.lastAveragedThroughput < 20000) {
                        //         rcqi = 1.79769e+308;

                        //     }
                        // }  else {

                        // if (rntiSAndQoSParams[(*it).first].m_qosBearerType == LogicalChannelConfigListElement_s::QBT_GBR)
                        // {
                        //     if(rntiSAndQoSParams[(*it).first].m_qci == 1) {
                        //         koef = 5;
                        //     }
                        ...
                        NS_LOG_INFO(this << " RNTI " << (*it).first << " MCS " << (uint32_t)mcs
                                         ...
                                         << rcqi);
                        //а теперь просто выбираем максимум!!!
                        if (rcqi > rcqiMax)
                        {
                            rcqiMax = rcqi;
                            itMax = it;
            ...
            //если вдргу не было максимума (никто ничего не передает, или RBS недоступны, то ничего не происходит)
            ...
            {
                //иначе мы говорим, что  i-тая RB группа выделена абоненту!!
                rbgMap.at(i) = true;
    ...
    while (itMap != allocationMap.end())
    {
        ...
        for (uint8_t j = 0; j < nLayer; j++)
        {
            ...
            // std::cout << tbSize << std::endl;
            // std::cout << rbgSize << std::endl;
        ...
        for (itBufReq = m_rlcBufferReq.begin(); itBufReq != m_rlcBufferReq.end(); itBufReq++)
        {
            ...
            //     std::cout << "TRANS QUEUE SIZE: " <<  (*itBufReq).second.m_rlcTransmissionQueueSize  << std::endl;
            //     std::cout << "RETRANS QUEUE SIZE: " << (*itBufReq).second.m_rlcRetransmissionQueueSize  << std::endl;
            //     std::cout << "HOL DELAY: " << (*itBufReq).second.m_rlcRetransmissionHolDelay << std::endl;
            //     std::cout << "TRANS QUEUE SIZE: " << std::endl;
            // }
            if (((*itBufReq).first.m_rnti == (*itMap).first) &&
