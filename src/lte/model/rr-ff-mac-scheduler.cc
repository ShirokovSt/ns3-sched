...
{
    std::cout << "sub: " << params.m_subframeAssignment << std::endl;
    NS_LOG_FUNCTION(this);
...
{ //0 - 10 0 - 4 - два абонента, 4 - 7 - 3 абонента 7 - 10 - два абонента

     if(Simulator::Now() == Seconds(2.010) || Simulator::Now() == Seconds(2.020) || Simulator::Now() == Seconds(2.030))  { //|| Simulator::Now() == Seconds(4.101) || Simulator::Now() == Seconds(4.111) || Simulator::Now() == Seconds(4.110)  ) { //&& Simulator::Now() < Seconds(7.0)) {
        for(auto it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
            if((*it).m_rnti == 1) {
                (*it).m_rlcTransmissionQueueSize = 1;
                break;
            }
        }
    ...
    //МЕТРИКА rbgPerTb - то сколько RGB будет выделяться каждому абоненту в данный TTI -
    // в нашем случае - RBG = 12 -> если 3 абонента, то каждому по 4 RBG (или 8 RB)
    // по сути nTbs - активные абоненты, если они есть, то берем 12 (всего RBG) минус выделенные RBG и делим на 3, получаем 4
    ...
    //выяснено, что в этом цикле 8 блоков как раз выделяется одному абоненту в нашем случае, далее эта функция
    //вызывается для следующего абонента
    do
    {
        itLcRnti = lcActivesPerRnti.find((*it).m_rnti);
        std::set<uint16_t>::iterator itRnti = rntiAllocated.find((*it).m_rnti);
        ...
        if(Simulator::Now() >= Seconds(2.000) && Simulator::Now() < Seconds(2.040)) {
            ...
            std::cout << "RNTI: " << (*it).m_rnti << std::endl;
            std::cout << "BLOCKS: " << rbgPerTb * rbgSize << std::endl;
        ...
        NS_LOG_INFO("RBG:");
        //тут как понимаю и выделяем вот эти вот RGB
        while (i < rbgPerTb)
        {
